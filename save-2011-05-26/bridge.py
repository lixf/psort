...
class Bridge:
    ...
    def __init__(self, master, count = 0, rhost = '', subdivisions = 1,
                 border = 0):
        self.master = master
        if count > 0:
            self.count = count
    ...
    def init(self):
        ...
        if self.sorteractive:
            ...
            self.sorter = psort.Psort(n = self.count,
                                      ...
                                      value = value, rhost = self.rhost,
                                      subdivisions = self.subdivisions,
                                      bordercolor = self.bordercolor)
        else:
            self.sorter = psort.Psort(self.count, rhost = self.rhost,
                                      subdivisions = self.subdivisions,
                                      bordercolor = self.bordercolor)
...
def run(name, args):
    ...
    border = 0
    subdivisions = 1
    optlist, args = getopt.getopt(args, 'hr:c:s:b:')
    ...
    print "Creating bridge with %d columns and with remote host '%s'" % (cols, rhost)
    if (subdivisions > 1):
        print "Subdivide rectangle into % d subrectangles.  Border color = %f" % (subdivisions, border)
    root = Tk()

    bridge = Bridge(root, cols, rhost, subdivisions, border)
    try:
        root.mainloop()
    except:
        exit()
